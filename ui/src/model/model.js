...
class Model extends Component {
  ...
  onMouseOver = (event) => {
    ...
    if (intersects.length > 0) {
      ...
      while (!object.name.startsWith("Sat_")) {
  ...
  getIntersects = (event) => {
    ...
    mouse.x = ((event.clientX - 200) / (this.mount.clientWidth)) * 2 - 1;
  ...
  clearGroup = (name) => {
    const group = this.state.group.getObjectByName(name)
    if (group) {
      group.clear()
      this.state.group.remove(group)
    }
  }
  ...
  update = () => {
    ...
    if (sateGroup && sateGroup.children.length > 0) { // 如果卫星组存在,直接更新卫星位置
      ...
      const pickedGround = this.props.pickedGroundStation;
      const pickedGroundStr = pickedGround.toString();
      if (pickedGroundStr !== this.lastGroundStationsStr) {
        this.lastGroundStationsStr = pickedGroundStr;
        this.clearGroup("groundGroup");
        this.clearGroup("groundLabelGroup");
        addGroundStations(this.props.groundPosition, this.state.group, this.props.chosenGround, this.props.pickedGroundStation);
      }
    } else { // 如果卫星组不存在，添加卫星
      // 删除原有的卫星组（此时卫星组内没有子元素）
      // console.log("Remove several groups!")
      this.clearGroup("sateGroup");
      this.clearGroup("labelGroup");
      this.clearGroup("groundGroup");
      this.clearGroup("groundLabelGroup");
    ...
    // 删除卫星连线、路由路径
    this.clearGroup("lineGroup");
    this.clearGroup("pathGroup");
    ...
    if (this.props.flags["connect"]) {
      ...
      addLinesBetweenSatelliteAndGroundStation(
          this.state.group, this.props.sateLocations, this.props.chosenGround, this.props.groundConnections);
    }
