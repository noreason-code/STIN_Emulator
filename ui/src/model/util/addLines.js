...
export const addLinesBetweenSatellites = (group, sateLocations, sateConnections) => {
   ...
   */
  if ( sateLocations == null || sateConnections == null ||
      Object.keys(sateLocations).length===0 || Object.keys(sateConnections).length===0 ) {
    return;
  }
  const lineGroup = new Group();
  // console.log(sateConnections)
  // console.log(sateLocations)
  for (let key in sateConnections) {
    if (!(key in sateLocations) || !(key in sateConnections)) {
      return
    }
...
export const addLinesBetweenSatelliteAndGroundStation = (group, sateLocations, groundPositions, groundConnections) => {
   ...
   */
    if ( sateLocations==null || groundPositions==null || groundConnections==null ||
        Object.keys(sateLocations).length===0 || Object.keys(groundPositions).length===0 || Object.keys(groundConnections).length===0) {
      return;
    }
    const lineGroup = group.getObjectByName("lineGroup");
    for (let key in groundConnections) {
      // 判断地面站是否与卫星连接，未连接时groundConnections[key]为空
      if (!(key in groundConnections) || groundConnections[key] === '') {
        continue;
      }
      // console.log(key)
