...
class DockerClient:
    ...
    def __init__(self, image_name: str, host_ip: str, ground_image_name: str, user_image_name: str):
    ...
    def create_satellite(self, node_id: str, port: str, satellite_num: int, successful_init) -> str:
        ...
        if successful_init:
            container_info = self.client.containers.run(self.image_name, detach=True, environment=[
                ...
                VOLUME1, VOLUME2, V_EDIT, VOLUME3, VOLUME_SK], privileged=True, ports={'8765/tcp': 30000+index})
        else:
            container_info = self.client.containers.run(self.image_name, detach=True, environment=[
                'NODE_ID=' + node_id,
                'HOST_IP=' + self.host_ip,
                ...
                VOLUME1, VOLUME2, V_EDIT, VOLUME3], privileged=True)
    ...
    def create_ground_container(self, node_id_g: str):
        container_info = self.client.containers.run(self.ground_image_name, detach=True, environment=[
                ...
                "MONITOR_IP=" + "172.17.0.2",
                "DISPLAY=unix:0.0",
            ], cap_add=['NET_ADMIN'], name=node_id_g, volumes=[VOLUME_IPERF], privileged=True)
        return container_info.id

    def create_user_container(self, node_id_u: str):
        container_info = self.client.containers.run(self.user_image_name, detach=True, environment=[
                'NODE_ID=' + node_id_u,
                'HOST_IP=' + self.host_ip,
                "MONITOR_IP=" + "172.17.0.2",
                "DISPLAY=unix:0.0",
            ], cap_add=['NET_ADMIN'], name=node_id_u, volumes=[VOLUME_IPERF], privileged=True, command=["tail","-f","/dev/null"])
        return container_info.id
    ...
    #hs add
    def connect_node_for_switch(self, container_id: str, network_id: str, alias_name: str):
        container_object = self.client.containers.get(container_id)
        self.client.networks.get(network_id).connect(container_object, aliases=[alias_name])
    ...
    def create_network(self, network_id: str):
        ...
        for i in range(100):
            try:
                subnet_ip = self.allocator.alloc_local_subnet(True, network_id)
                subnet_ip_str = ip2str(subnet_ip)
                gateway_str = ip2str(subnet_ip + 1)
                print(f'network_id:{network_id}')
                print(f'subnet_ip:{subnet_ip_str}')
                print(f'gateway_str:{gateway_str}')
