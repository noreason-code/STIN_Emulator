...
def generate_tle(orbit_num: int, orbit_satellite_num: int, latitude, longitude, delta, period) -> (list, dict):
    ...
    line_2 = "2 00000  86.4000 %08.4f 0000011   0.0000 %8.4f %11.8f00000"
    ...
    year2, day = get_year_day(base_datetime)
...
if __name__ == "__main__":
    satellites, topo = generate_tle(6, 11, 0, 0, 0, 0.1)
    # print(np.array(satellites))
    # print(topo)
    with open('./base_datetime.txt', 'r') as f:
        datetime_str = f.readline()
    datetime_format = '%Y-%m-%d %H:%M:%S.%f'
    base_datetime = datetime.strptime(datetime_str, datetime_format)
    year, day = get_year_day(base_datetime)
    print(year)
    print(day)
